export class ReactiveNode {
    private static resolveNode(node: Node): Node[] {
        return node instanceof DocumentFragment
            ? Array.from(node.childNodes)
            : [node];
    }

    private children: Node[] = [document.createComment('')];
    ...
    public update(node: Node | null) {
        ...
        if (parentNode) {
            const newChildren: Node[] = Array.from(parentNode.childNodes);
            newChildren.splice(
                newChildren.indexOf(firstChild),
